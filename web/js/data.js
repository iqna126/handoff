...
async function runFull(queryFn) {
...
// 清空数据（我的 → 数据）：按用户勾选的分类清，不是一键清空所有表——
// 用户可能只想清掉训练记录留着 PR，或者反过来。表名跟"我的 → 数据"页面
// 展示的分类一一对应。
...
// { table: 行数 }——用来在清空前的确认页面上显示"即将删除 N 条"，让用户
// 知道自己到底在删多少东西，不是盲选分类名字
export async function countUserData() {
  const counts = {};
  for (const table of Object.keys(CLEARABLE_TABLES)) {
    const result = await runFull(() => supabase.from(table).select("*", { count: "exact", head: true }));
    counts[table] = result.count || 0;
  }
  return counts;
}

// tables：要清空的表名数组（CLEARABLE_TABLES 的 key 子集）。RLS 已经把
// 每张表限制到"只能碰自己的行"，这里的 .delete() 天然只删自己的数据；
// 不带任何 .eq() 条件的裸 delete 会被 supabase-js 拒绝（"missing WHERE
// clause"的安全保护），所以用一个恒真的 .neq() 占位，不是真的在按 id 过滤。
export async function clearUserData(tables) {

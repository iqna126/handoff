...
| 3 | wodify-pull 主路径上线（见 §6.6/§6.9） | 🚧 代码全部写完且有测试（`/api/wod/ingest`、独立存活校验、`sync.py`/`cli.py`/`cdp.py`）；`cdp.py` 还没真机验证过（需要真实登录的 Chrome，见 `ops/cron-box-setup.md`「prime 在哪里做」——设计上是在自己电脑上跑，不在常开机器上）。甲骨文云机器还没注册（卡在客服审核） |
